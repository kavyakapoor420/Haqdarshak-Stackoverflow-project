...
class EnhancedQueryRequest(BaseModel):
    query: str
    language_code: Optional[str] = 'en-IN'
    chat_history: Optional[List[Dict]] = []  # Add chat history
...
def extract_scheme_from_history_and_query(query: str, chat_history: List[Dict]) -> Optional[str]:
    """
    Extract scheme name from current query and chat history context
    """
    example_schemes = [
        "Sukanya Samriddhi Account Scheme",
        "Pradhan Mantri Awas Yojana", 
    ...
    query_prompt = f"""
    You are given a user query about government schemes.
    Your task is to identify and extract the exact name of the scheme mentioned in the query.
    The scheme name is a proper noun or specific title, such as one of the following examples:
    {chr(10).join(['- ' + scheme for scheme in example_schemes])}
    ...
    try:
        ...
        scheme_from_query = response.text.strip()
        if scheme_from_query and scheme_from_query.lower() != "null":
            logger.info(f"Found scheme in current query: {scheme_from_query}")
            return scheme_from_query
    except Exception as e:
        logger.error(f"Error extracting scheme from query: {e}")
    
    # If no scheme found in current query, check recent chat history
    if chat_history:
        # Get last 6 messages (3 exchanges) to maintain context
        ...
        if history_context.strip():
            history_prompt = f"""
            You are given a current user query and recent chat history.
            Your task is to identify if there's a government scheme being discussed in the conversation context.
            
            Look for scheme names mentioned in the chat history that the current query might be referring to.
            Common scheme examples:
            {chr(10).join(['- ' + scheme for scheme in example_schemes])}
            ...
            Based on the context, what scheme is being discussed? 
            If no scheme can be identified from the context, return "null".
            Provide only the scheme name as a string, or "null" if not found.
            ...
            try:
                response = gemini_model.generate_content(history_prompt)
                scheme_from_history = response.text.strip()
                if scheme_from_history and scheme_from_history.lower() != "null":
                    logger.info(f"Found scheme from chat history: {scheme_from_history}")
                    return scheme_from_history
            except Exception as e:
                logger.error(f"Error extracting scheme from history: {e}")
...
@app.post("/rag-query", response_model=RAGResponse)
async def rag_query_enhanced(query: EnhancedQueryRequest):
    try:
        logger.info(f"Processing enhanced RAG query with memory: {query.query}")
        ...
        # Step 2: Enhanced scheme extraction with memory
        scheme_name = extract_scheme_from_history_and_query(translated_query, query.chat_history or [])
        
        if not scheme_name:
            logger.warning(f"No scheme name found for query: {query.query}")
            response_text = "Please specify the scheme name in your query or refer to a scheme mentioned earlier in our conversation."
            ...
            return RAGResponse(
                success=False,
                message="No scheme name found. Please specify the scheme name for a better response.",
                scheme_name=None,
                response=response_text,
                retrieved_chunks=[],
                detected_language=detected_language,
        ...
        logger.info(f"Using scheme: {scheme_name} (from query or context)")
        ...
        # Step 5: Generate response using retrieved chunks with conversation context
        if not retrieved_chunks:
            response_text = f"I couldn't find specific information about {scheme_name}. Please make sure the scheme name is correct or try uploading the relevant document first."
        ...
        if query.chat_history:
            recent_messages = query.chat_history[-4:]  # Last 2 exchanges
            for msg in recent_messages:
                if msg.get('role') and msg.get('content'):
                    conversation_context += f"{msg['role'].title()}: {msg['content']}\n"
        
        # Enhanced response generation prompt with conversation memory
        response_prompt = f"""
        You are a helpful chatbot answering questions about government schemes. 
        Use the following context to answer the user's query accurately and concisely in English.
        The user's original query was in a different language, but provide your answer in clear English first.
        
        IMPORTANT: Consider the conversation history to provide contextual follow-up answers.
        If this appears to be a follow-up question about a scheme discussed earlier, reference that context.
        
        Scheme Information Context:
        {context}
        ...
        Translated Query: {translated_query}
        Identified Scheme: {scheme_name}
        
        Provide a comprehensive answer based on the context and conversation history. 
        If this is a follow-up question, acknowledge the context from our previous discussion.
        If the context doesn't contain enough information, provide what information is available 
        and suggest clarifying the query:
        """
        ...
        try:
            ...
            response_text = response.text.strip()
            logger.info("Generated English response with conversation context successfully")
        ...
        return RAGResponse(
            success=True,
            message="Enhanced query with conversation memory processed successfully",
            scheme_name=scheme_name,
            response=response_text,
            retrieved_chunks=retrieved_chunks,
            detected_language=detected_language,
        ...
        logger.error(f"Unexpected error processing enhanced RAG query: {e}")
        raise HTTPException(status_code=500, detail=f"Error processing query: {str(e)}")
